...
export const setupChatSocket = (io) => {
    ...
    io.on('connection', (socket) => {
        console.log(`사용자 연결됨: ${socket.memberId}`);

        // 채팅방 입장
        socket.on('join-room', async (chatRoomId) => {
            try {
                // 채팅방 접근 권한 확인
                const isMember = await ChatModel.isChatRoomMember(chatRoomId, socket.memberId);
                ...
                console.log(`사용자 ${socket.memberId}가 채팅방 ${chatRoomId}에 입장`);
        ...
        socket.on('leave-room', (chatRoomId) => {
            ...
            console.log(`사용자 ${socket.memberId}가 채팅방 ${chatRoomId}에서 퇴장`);
        ...
        socket.on('send-message', async ({ chatRoomId, content }) => {
            try {
                ...
                // 데이터베이스에 메시지 저장
                const message = await ChatModel.sendMessage(chatRoomId, socket.memberId, content.trim());

                // 메시지에 발신자 정보 추가
                const messageWithSender = {
                    ...message,
                    sender_id: socket.memberId,
                };
        ...
        socket.on('mark-as-read', async ({ chatRoomId }) => {
            try {
                ...
                // 읽음 처리
                const count = await ChatModel.markMessagesAsRead(chatRoomId, socket.memberId);
                ...
                console.log(`읽음 처리: 사용자 ${socket.memberId}, 채팅방 ${chatRoomId}, ${count}개 메시지`);
            } catch (error) {
        ...
        socket.on('disconnect', () => {
            console.log(`사용자 연결 해제됨: ${socket.memberId}`);
        });
